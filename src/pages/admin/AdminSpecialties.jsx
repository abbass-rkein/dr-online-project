...
export default function AdminSpecialties() {
  ...
  async function load(nextPage = page, nextQ = q) {
    ...
    try {
      ...
      const r = await fetch(`${API}/api/admin/specialties?${qs.toString()}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const j = await r.json();
      if (!r.ok || !j.ok)
        throw new Error(j.error || "Failed to load specialties");
  ...
  async function save() {
    ...
    try {
      ...
      const url = edit
        ? `${API}/api/admin/specialties/${edit.specialty_id}`
        : `${API}/api/admin/specialties`;
  ...
  async function delRow(rw) {
    const ok = confirm(
      rw.doctor_count > 0
        ? `This specialty is used by ${rw.doctor_count} doctors and cannot be deleted.`
    ...
    try {
      const r = await fetch(`${API}/api/admin/specialties/${rw.specialty_id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      const j = await r.json();
      ...
      setRows((prev) => prev.filter((x) => x.specialty_id !== rw.specialty_id));
    } catch (e) {
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Manage specialties used for doctor profiles and filtering.
          </p>
        </div>
        ...
        <table className="w-full min-w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              rows.map((r) => (
                <tr key={r.specialty_id} className="hover:bg-slate-50/60">
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-lg font-bold text-slate-900">
                  {edit ? "Edit Specialty" : "Add Specialty"}
                </h2>
